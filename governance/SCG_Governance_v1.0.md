...
**Status:** FROZEN  
**Applies To:** SCG (authoritative runtime), downstream governance mirrors including Iter MCP extension  

---

## Integrity Lock
This manifest is the sole authoritative governance artifact for SCG.
The canonical checksum source is `governance/governance.hash`.
Downstream repositories may mirror this file, but must not define governance.
Because v1.0 is frozen, future governance evolution must create a new versioned
manifest rather than mutating this file in place.
...
- **Trigger:** User inside directory with Cargo.toml, Dockerfile, pyproject.toml, requirements.txt, package.json, or SCG spec files
...
- **Trigger:** When adding, editing, or deleting project rules
