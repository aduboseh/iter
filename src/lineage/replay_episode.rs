...
impl ReplayEpisode {
    ...
    pub fn validate(&mut self) -> Result<(), String> {
        ...
        if self.environments.len() < 3 {
            return Err(
                "Replay protocol requires 3 environments (local, docker, kubernetes)".into(),
            );
        }
...
pub fn generate_test_scenario(seed: u64) -> Vec<String> {
    ...
    for _ in 0..250 {
        ...
        let operation = match op_type {
            0 => format!(
                "node.create(belief={}, energy={})",
                (rng_state % 100) as f64 / 100.0,
                (rng_state % 10) as f64
            ),
            1 => format!(
                "node.mutate(id=node_{}, delta={})",
                (rng_state % 10),
                ((rng_state % 20) as f64 - 10.0) / 100.0
            ),
            2 => format!(
                "edge.bind(src=node_{}, dst=node_{}, weight={})",
                (rng_state % 10),
                ((rng_state / 10) % 10),
                (rng_state % 100) as f64 / 100.0
            ),
            3 => format!("edge.propagate(edge_id=edge_{})", rng_state % 5),
            4 => "governor.status()".to_string(),
            _ => unreachable!(),
        };
...
mod tests {
    ...
    fn test_replay_protocol() {
        ...
        protocol
            .update_episode("REPLAY_001", "local", "config.json", "Linux", hash)
            .unwrap();
        protocol
            .update_episode("REPLAY_001", "docker", "image", "Linux", hash)
            .unwrap();
        protocol
            .update_episode("REPLAY_001", "k8s", "cluster", "Linux", hash)
            .unwrap();
