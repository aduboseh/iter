...
/// Input bundle to avoid too_many_arguments on create
#[derive(Debug, Clone)]
...
impl LineageSnapshot {
    ...
    pub fn create(args: CreateArgs) -> Self {
        ...
        let lineage_hash = Self::compute_lineage_hash(&args.entries);

        // Compute topology hash (for cycle detection)
        let topology_hash = Self::compute_topology_hash(args.node_count, args.edge_count);
    ...
    pub fn export_to_file<P: AsRef<Path>>(&self, path: P) -> Result<String, String> {
        ...
        eprintln!(
            "[SNAPSHOT] Exported to {:?} with hash {}",
            path.as_ref(),
            file_hash
        );
    ...
    pub fn import_from_file<P: AsRef<Path>>(path: P) -> Result<Self, String> {
        let json = fs::read_to_string(&path).map_err(|e| format!("File read failed: {}", e))?;
        ...
        if computed_hash != snapshot.lineage_hash {
            return Err(format!(
                ...
                snapshot.lineage_hash, computed_hash
            ));
        }

        eprintln!(
            "[SNAPSHOT] Imported from {:?} - hash verified",
            path.as_ref()
        );
    ...
    fn hash_file<P: AsRef<Path>>(path: P) -> Result<String, String> {
        let contents = fs::read(&path).map_err(|e| format!("File read failed: {}", e))?;
...
impl LineageBuilder {
    ...
    pub fn add_entry(&mut self, operation: impl Into<String>, params: serde_json::Value) {
...
mod tests {
    ...
    fn test_snapshot_creation() {
        ...
        builder.add_entry(
            "node.create",
            serde_json::json!({"belief": 0.5, "energy": 1.0}),
        );
