...
/// Type alias to simplify callback type
pub type TelemetryCallback = Box<dyn Fn(&TelemetryRecord) + Send + Sync>;

/// Thread-safe telemetry emitter with buffering.
pub struct TelemetryEmitter {
    cluster_id: String,
    buffer: parking_lot::Mutex<Vec<TelemetryRecord>>,
    emit_callback: Option<TelemetryCallback>,
}
...
pub fn with_callback<F>(mut self, callback: F) -> Self
    ...
    {
        self.emit_callback = Some(Box::new(callback) as TelemetryCallback);
        self
    }
    ...
    pub fn emit(
        ...
        // Emit to stderr for local debugging (disabled in determinism mode)
        if std::env::var("SCG_DETERMINISM").is_err() {
