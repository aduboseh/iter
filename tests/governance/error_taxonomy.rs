...
fn all_error_variants_have_codes() {
    ...
    let errors = vec![
        ...
        McpError::EsvValidationFailed { reason: String::new() },
        McpError::DriftExceeded { drift: 0.0, threshold: 0.0 },
        McpError::LineageCorruption { details: String::new() },
        McpError::SubstrateError { message: String::new() },
        McpError::BadRequest { message: String::new() },
    ];
    ...
    for err in &errors {
        assert!(!err.code_string().is_empty(), "Error {:?} has empty code_string", err);
    }
}
...
fn error_codes_match_expected() {
    let errors: Vec<(McpError, &str, u32)> = vec![
        ...
        (McpError::EsvValidationFailed { reason: String::new() }, "esv_validation_failed", 1000),
        (McpError::DriftExceeded { drift: 0.0, threshold: 0.0 }, "drift_exceeded", 2000),
        (McpError::LineageCorruption { details: String::new() }, "lineage_corruption", 3000),
        (McpError::SubstrateError { message: String::new() }, "substrate_error", 5000),
        (McpError::BadRequest { message: String::new() }, "bad_request", 4000),
    ];
    
    for (err, expected_code_str, expected_code) in errors {
        assert_eq!(
            err.code_string(), expected_code_str,
            "Error {:?} has wrong code_string", err
        );
        assert_eq!(
            err.code(), expected_code,
            "Error {:?} has wrong numeric code", err
        );
    }
}
...
fn error_codes_are_documented() {
    ...
    for (expected_code, _) in EXPECTED_ERRORS {
        assert!(
            ...
            "Expected error code '{}' not found in McpError variants", expected_code
        );
    }
}
...
fn variant_count_matches_expected() {
    ...
    assert_eq!(
        EXPECTED_ERRORS.len(), variant_count,
...
fn errors_have_meaningful_display() {
    let errors = vec![
        ...
        (McpError::EsvValidationFailed { reason: "test".into() }, "test"),
        (McpError::DriftExceeded { drift: 0.5, threshold: 0.1 }, "0.5"),
        (McpError::LineageCorruption { details: "corrupt".into() }, "corrupt"),
        (McpError::SubstrateError { message: "oops".into() }, "oops"),
        (McpError::BadRequest { message: "invalid".into() }, "invalid"),
    ];
    ...
    for (err, expected_substring) in errors {
        ...
        assert!(
            ...
            "Error display '{}' should contain '{}'", display, expected_substring
        );
    }
}
...
fn error_code_conversion_is_consistent() {
    ...
    for err in errors {
        ...
        assert_eq!(
            err.error_code(),
            err.code() as i32,
            "error_code() and code() mismatch for {:?}", err
        );
    }
}
...
fn errors_serialize_to_json() {
    let errors = vec![
        ...
        McpError::BadRequest { message: "test".into() },
    ];
